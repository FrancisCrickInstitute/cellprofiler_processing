...
class CellPaintingAnalysis:
    ...
    def __init__(self):
        ...
        self.data_path = "/nemo/stp/hts/working/Phenix/Joe_Stan_Astrocytes_cell_paint_30062025/cellprofiler/processed_data_3/data/processed_image_data_well_level.parquet"
        self.output_dir = "/nemo/stp/hts/working/Phenix/Joe_Stan_Astrocytes_cell_paint_30062025/cellprofiler/processed_data_3/data/feature_analysis"
        self.df = None
        ...
        # Define control and mutant lines
        self.control_lines = ['CTRL1', 'CTRL2', 'CTRL3', 'CTRL5', 'CTRL6']
        self.mutant_lines = ['CB1D', 'CB1E', 'NC2', 'GliA', 'GliB']
    ...
    def quality_control(self, missing_threshold=0.05, remove_extreme_features=True):
    ...
    def identify_extreme_features(self, threshold_multiplier=100):
        """
        Identify features with extreme mean differences that may be artifacts
        
        Parameters:
        threshold_multiplier: Features with mean differences > threshold_multiplier * median absolute difference
                            are flagged as extreme
        """
        print("\nIdentifying features with extreme values...")
    ...
    def perform_comparisons_vs_control(self, fdr_threshold=0.1):
    ...
    def create_summary_plot(self, save_plots=True):
        ...
        if save_plots:
            plt.savefig(f'{plot_dir}/significant_features_summary.png', dpi=300, bbox_inches='tight')
        plt.show()
    ...
    def create_top_features_plot(self, comparison='ALS_Untreated_vs_Control', top_n=8, save_plots=True):
        ...
        group_colors = {
            'Control Untreated': '#2E8B57',      # Sea Green
            'Control SB431542': '#4682B4',       # Steel Blue  
            'ALS Untreated': '#DC143C',          # Crimson
            'ALS SB431542': '#FF6347'            # Tomato
        }
    ...
    def create_single_feature_plots(self, comparison='ALS_Untreated_vs_Control', top_n=10, save_plots=True):
        ...
        plot_dir = os.path.join(self.output_dir, 'plots', 'individual_features')
        if save_plots:
            os.makedirs(plot_dir, exist_ok=True)
    ...
    def create_heatmap(self, top_n_per_comparison=5, save_plots=True):
        ...
        plot_dir = os.path.join(self.output_dir, 'plots')
        if save_plots:
            plt.savefig(f'{plot_dir}/features_heatmap.png', dpi=300, bbox_inches='tight')
        plt.show()
        
        return self
    

    def create_mean_difference_heatmap(self, top_n=50, save_plots=True):
        """
        Create a mean difference heatmap showing top differential features across all conditions vs Control
        
        Parameters:
        top_n: int, number of top features to display (ranked by maximum absolute mean difference)
        ...
        if save_plots:
            plt.savefig(f'{plot_dir}/mean_difference_heatmap_top{top_n}.png', dpi=300, bbox_inches='tight')
    ...
    def create_effect_size_summary(self, min_effect_size=0.5, save_plots=True):
        ...
        if save_plots:
            plt.savefig(f'{plot_dir}/effect_size_summary.png', dpi=300, bbox_inches='tight')
    ...
    def run_analysis(self):
        ...
        (self.load_data()
        .quality_control(missing_threshold=0.05, remove_extreme_features=True)
        .create_biological_replicates()
        .perform_comparisons_vs_control(fdr_threshold=0.05)
        .identify_extreme_features(threshold_multiplier=100)
        .create_summary_plot()
        .create_volcano_plots()
        .create_mean_difference_heatmap(top_n=50)  # Top 50 features
        .create_effect_size_summary(min_effect_size=0.5)
        .create_top_features_plot('ALS_Untreated_vs_Control', top_n=8)
